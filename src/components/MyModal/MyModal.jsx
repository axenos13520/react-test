...
export default function MyModal({ children, visible, setVisible, title = "" }) {
  ...
  return (
    ...
    >
      <div
        className={
          `transition-opacity duration-300 ease-out absolute p-6 pt-10 min-w-64 h-min rounded-lg bg-slate-200 border-4 border-slate-400 shadow-lg shadow-gray-700/20` +
          windowClasses.join(" ")
        }
        onMouseDown={OnMouseDown}
        style={{ left: position[0], top: position[1], opacity: windowOpacity }}
      >
        <h2 className="absolute left-2 top-1 text-sm font-semibold text-slate-500">
          {title}
        </h2>
